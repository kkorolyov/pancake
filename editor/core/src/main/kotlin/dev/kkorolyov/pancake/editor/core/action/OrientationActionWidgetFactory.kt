...
import dev.kkorolyov.pancake.core.action.OrientationAction
...
class OrientationActionWidgetFactory : ActionWidgetFactory {
	override fun get(t: Action): Widget? = ActionWidgetFactory.get<OrientationAction>(t) {
		Widget { text(this) }
	}
	...
	override fun get(c: Class<Action>, onNew: (Action) -> Unit): Widget? = ActionWidgetFactory.get(c, onNew) {
		...
		Widget {
			...
			tooltip("value")
			button("apply") { it(OrientationAction(value)) }
		}
	}
}
