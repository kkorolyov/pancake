...
class VelocityActionWidgetFactory : ActionWidgetFactory {
	override fun get(t: Action): Widget? = ActionWidgetFactory.get<VelocityAction>(t) {
		Widget { text(this) }
	}
	...
	override fun get(c: Class<Action>, onNew: (Action) -> Unit): Widget? = ActionWidgetFactory.get(c, onNew) {
		...
		Widget {
			...
			tooltip("value")
			button("apply") { it(VelocityAction(value)) }
		}
	}
}
