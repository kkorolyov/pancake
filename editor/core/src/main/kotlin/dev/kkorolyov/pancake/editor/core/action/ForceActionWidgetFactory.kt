...
class ForceActionWidgetFactory : ActionWidgetFactory {
	override fun get(t: Action): Widget? = ActionWidgetFactory.get<ForceAction>(t) {
		Widget { text(this) }
	}
	...
	override fun get(c: Class<Action>, onNew: (Action) -> Unit): Widget? = ActionWidgetFactory.get(c, onNew) {
		...
		Widget {
			...
			tooltip("value")
			button("apply") { it(ForceAction(value)) }
		}
	}
}
