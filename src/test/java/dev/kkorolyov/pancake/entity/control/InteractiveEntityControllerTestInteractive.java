...
public class InteractiveEntityControllerTestInteractive extends Application {
	...
	private static EntityController buildController() {
		...
		controller.setActions(new KeyAction(e -> e.getBody().addForce(0, -10), null, e -> e.getBody().addForce(0, 10), KeyCode.W),
													new KeyAction(e -> e.getBody().addForce(-10, 0), null, e -> e.getBody().addForce(10, 0), KeyCode.A),
													new KeyAction(e -> e.getBody().addForce(0, 10), null, e -> e.getBody().addForce(0, -10), KeyCode.S),
													new KeyAction(e -> e.getBody().addForce(10, 0), null, e -> e.getBody().addForce(-10, 0), KeyCode.D),
													new KeyAction(e -> e.getBody().setMaxSpeed(10), null, e -> e.getBody().setMaxSpeed(5), KeyCode.SHIFT),
													new KeyAction(e -> e.getBounds().getOrigin().set(0, 0), null, null, KeyCode.ESCAPE));
