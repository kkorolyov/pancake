 ...
 */
fun getSnapshot(c: Class<out Mesh>): Snapshot = factories.firstNotNullOfOrNull { it.get(c) } ?: throw UnsupportedOperationException("no provider found for mesh [$c]")
...
interface SnapshotFactory {
	/**
	 * Returns a snapshot for [c]-type meshes, if this factory handles it.
	 */
	fun get(c: Class<out Mesh>): Snapshot?
}
